...
- [ ] **[1.1.1] 아이콘 버튼** — 텍스트 없는 버튼/아이콘에 `aria-label` 또는 `.sr-only` 제공
...
- [ ] **[1.3.1] 색에 무관한 인식** — 색상만으로 정보 전달 금지 (에러: 색+아이콘+텍스트 병행)
- [ ] **[1.3.2] 명확한 지시사항** — "빨간색 버튼을 클릭", "왼쪽 메뉴" 등 감각적 표현만 사용 금지
- [ ] **[1.3.3] 텍스트 명도 대비** — 일반 텍스트 **4.5:1 이상**, 큰 텍스트(18px+ / bold 14px+) **3:1 이상**
- [ ] **[1.3.3] UI 컴포넌트 대비** — 버튼 테두리·입력 필드 경계 등 **3:1 이상**
- [ ] **[1.3.3] placeholder 대비** — **4.5:1 이상** (한국 기준 강화)
- [ ] **[1.3.4] 자동 재생 금지** — 3초 이상 자동 재생 콘텐츠는 정지·일시정지·음소거 수단 제공
- [ ] **[1.3.5] 콘텐츠 간의 구분** — 인접 콘텐츠 간 시각적 구분 (여백·선·배경색 등)
...
- [ ] **[2.1.1] 키보드 사용 보장** — 마우스 없이 Tab·Enter·Space·방향키만으로 모든 기능 접근
- [ ] **[2.1.1] 키보드 트랩 없음** — 포커스가 특정 영역에 갇히지 않아야 함 (모달 제외)
- [ ] **[2.1.2] 초점 이동과 표시** — 포커스 인디케이터 항상 **시각적으로 표시** (outline 제거 금지)
- [ ] **[2.1.2] 포커스 순서** — 논리적·시각적 흐름과 일치하는 Tab 순서
- [ ] **[2.1.2] 포커스 대비** — 포커스 인디케이터 대비율 **3:1 이상**
- [ ] **[2.1.3] 조작 가능** — 모바일 터치 타겟 최소 **44×44px**
- [ ] **[2.1.3] 스킵 내비게이션** — 페이지 최상단에 "본문 바로가기" 링크 제공
...
- [ ] **[2.3.2] prefers-reduced-motion** — 애니메이션·전환 효과에 `prefers-reduced-motion: reduce` 대응

### 지침 2.4 쉬운 내비게이션
> 콘텐츠를 쉽게 찾고 이동할 수 있어야 한다.

- [ ] **[2.4.1] 반복 영역 건너뜀** — GNB 등 반복 영역을 건너뛰는 "본문 바로가기" 제공
- [ ] **[2.4.2] 페이지 제목 제공** — `<title>` 태그에 페이지를 설명하는 고유 제목 제공
- [ ] **[2.4.3] 적절한 링크 텍스트** — "여기 클릭", "더보기" 단독 사용 금지 → 맥락 포함한 텍스트
- [ ] **[2.4.4] 제목 계층 구조** — `h1` → `h2` → `h3` 순서, 건너뜀 금지, 페이지당 `h1` 하나
...
- [ ] **[2.5.3] 레이블과 네임 일치** — 보이는 레이블 텍스트가 accessible name에 포함되어야 함
...
- [ ] **[3.1.1] 기본 언어 표시** — `<html lang="ko">` 설정 (현재 적용됨 ✅)
...
- [ ] **[3.1.4] 텍스트를 이미지로 사용 금지** — 로고 제외, 모든 텍스트는 실제 텍스트로
...
- [ ] **[4.1.1] 마크업 오류 방지** — 시작/종료 태그 쌍 일치, 중복 속성 없음, id 중복 없음
- [ ] **[4.1.1] HTML 유효성 검사** — [W3C Validator](https://validator.w3.org/) 통과
- [ ] **[4.1.2] 속성값 따옴표** — 모든 속성값은 따옴표로 감싸기

### 지침 4.2 웹 애플리케이션 접근성
> 웹 애플리케이션은 접근성이 있어야 한다.

- [ ] **[4.2.1] 스크린리더 호환** — NVDA(Windows), VoiceOver(Mac/iOS), TalkBack(Android) 테스트
- [ ] **[4.2.2] 커스텀 컴포넌트 role** — 네이티브 HTML로 불가한 경우만 ARIA role 사용
- [ ] **[4.2.3] 상태 정보 제공** — `aria-expanded`, `aria-selected`, `aria-checked`, `aria-pressed` 상태 반영
- [ ] **[4.2.4] 동적 콘텐츠 알림** — 변경된 콘텐츠는 `aria-live="polite"` 또는 `role="alert"`로 공지
...
```css
/* ✅ 포커스 스타일 — outline 제거 절대 금지, 커스텀으로 대체 */
:focus-visible {
  outline: 2px solid var(--primitive-green-500);
  outline-offset: 2px;
}
