...
- **TIER 3 `--color-*` 슬러그에는 Tailwind 유틸 접두(`text`·`bg`·`border`)를 넣지 않는다.** 유틸리티는 `{접두}-{슬러그}`로 조합되므로, 슬러그에 접두가 있으면 `text-text-muted`·`border-border`처럼 이중 접두가 생긴다.
  ...
  - **경계(border) 패밀리** — 구조 경계선은 `border-default`·`border-strong`·`border-brand`·`border-subtle`, 상태 경계선은 `border-danger`·`border-attention`·`border-success`·`border-disabled` 유틸리티. TIER 2 `--ds-border`·`--ds-border-strong`·`--ds-border-brand`·`--ds-border-subtle`·`--ds-border-{danger,attention,success,disabled}` → TIER 3 `--color-default`·`--color-strong`·`--color-brand`·`--color-subtle`·`--color-{danger,attention,success,disabled}`(@theme). 포커스 링은 border가 아닌 **Utility `utility-focus-ring`**. 큐레이션: `Border` 카테고리(`brand` / `neutral`=default·strong·subtle / `status`=danger·attention·success·disabled 그룹). 상태 경계는 UI 3:1만 필요하지만 전경 status 와 색을 맞추려 danger·attention·success 는 **라이트 50·다크 `.dark`에서 70(→raw 30) 재매핑**(텍스트·경계 동일 색)·disabled 은 비활성 저대비(gray-10, .dark gray-90 자동 반사).
  - 포커스 링 색은 TIER 1 hue raw 팔레트 → TIER 2 `--ds-utility-focus-ring` → TIER 3 `--color-utility-focus-ring` → 유틸 `utility-focus-ring`. 라이트 `--raw-teal-50` · 다크 `--raw-orange-30`. **utility** 접두 + **focus-ring** 기능명으로 관리한다(`ring` 단독 슬러그 금지).
...
- 사이드메뉴 프리셋은 `layout-sidenav` + `layout-sidenav-menu` + `layout-sidenav-content` 조합입니다. `layout-sidenav`는 래퍼로 lg부터 메뉴 너비(16rem)만큼 `padding-left`를 적용합니다. `layout-sidenav-menu`는 lg에서 `fixed`로 고정되며 헤더 높이(`--layout-header-height`) 아래에 위치합니다. 콘텐츠 좌우 스크린 마진·container 상한은 `layout-sidenav-content`가 담당합니다(`layout-page`와 동일). 넓은 menu는 `layout-sidenav-wide`입니다.
...
- 포커스 링은 전역 `:focus-visible` **outline**(`2.5px dashed` · `--ds-utility-focus-ring` / `utility-focus-ring`, offset `0.125rem`). 라이트 `--raw-teal-50` · 다크 `--raw-orange-30`. `role="tab"`·`.guide-toc`는 **outline-offset: -2px**. outline 제거·`outline-none` 금지.
