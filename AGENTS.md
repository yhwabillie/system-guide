...
- 반응형 그리드 시스템은 `small`(360px–, 4열, 가터 16px, 스크린 마진 16px) → `medium`(768px–, 8열, 가터 16px, 스크린 마진 24px) → `large`(1024px–, 12열, 가터 24px, 스크린 마진 24px) → `xlarge`(1280px–, large와 동일) → `xxlarge`(1440px–, large와 동일) tier로 구분합니다. tier 정의는 [`src/lib/layout-tokens.ts`](src/lib/layout-tokens.ts)의 `gridSystemTiers`가 SSOT이며, Tailwind 원시 breakpoint(`sm/md/lg/xl/2xl`)와는 별도 레이어입니다(small=prefix 없는 base, medium=`md:`, large=`lg:`, xlarge=`xl:`, xxlarge=`2xl:`). `2xl`(90rem/1440px) breakpoint는 `--layout-breakpoint-2xl`·`--breakpoint-2xl`로 정의되어 있습니다.
